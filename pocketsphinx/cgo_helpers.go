...
// NewSearchIterRef initialises a new struct.
func NewSearchIterRef(ref *C.ps_search_iter_t) *SearchIter {
	return (*SearchIter)(unsafe.Pointer(ref))
}
...
// unpackArgSSInt transforms a sliced Go data structure into plain C format.
func unpackArgSSInt(x [][]int16) (unpacked **C.int16, allocs *cgoAllocMap) {
	if x == nil {
		return nil, nil
	}
	allocs = new(cgoAllocMap)
...
// packSSInt reads sliced Go data structure out from plain C format.
func packSSInt(v [][]int16, ptr0 **C.int16) {
	const m = 0x7fffffff
	for i0 := range v {
